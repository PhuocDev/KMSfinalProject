...
public class JwtService {
    ...
    public Map<String,String> getInfoFromJWT(String token)
    {
        ...
        try {
            //kiểm tra sign key (phần cuối của jwt) có hợp lệ hay không bằng seccret
            // nếu không validate signkey có thể thay đổi được phần sign của token nhận vào
            Algorithm algorithm = Algorithm.HMAC256(jwtSecret);
            JWT.require(algorithm).withIssuer(issuer).build().verify(authToken);
            System.out.println("From validateToken in jwtService: Token hợp lệ");
