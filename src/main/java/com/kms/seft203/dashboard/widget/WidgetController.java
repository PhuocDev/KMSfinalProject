...
//    }
    @PostMapping({"/add/{id}"})
    public ResponseEntity<?> insertNewWidget(@RequestBody Widget widget, @PathVariable("id") String dashboardId) {
        if (widget != null && widget.getTitle()!= null){
            Dashboard dashboard = dashboardService.getDashboardById(dashboardId);
            if (dashboard==null) {
                return new ResponseEntity<String>("Not found dashboard id " + dashboardId, HttpStatus.NOT_FOUND);
            } else {
